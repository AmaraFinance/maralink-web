...
export default {
  ...
  asDouble(value, factor, decimal) {
    ...
    let _value;
    if (decimal) {
      _value = this.toFixed(Number(value) / Math.pow(10, decimal)).toString();
      return _value.indexOf('.') > 0 ? (_value.substr(0, _value.indexOf('.') + _factor + 1)) : (
        _value);
    } else {
      _value = this.toFixed(value).toString();
      return _value.indexOf('.') > 0 ? (_value.substr(0, _value.indexOf('.') + _factor + 1)) : (_factor>0?_value + '.' + '0000000000000'.substr(0, _factor):_value);
    }
  },
